...
$cart_results = $wpdb->get_results($wpdb->prepare("SELECT user_id, is_express FROM $custom_cart_session_table_name WHERE cart_hash_id = %s", $cartHashId));

foreach ($cart_results as $cart_result) {
   $orderId = $cart_result->user_id;
